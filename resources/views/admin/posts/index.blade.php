...
@section('content')
    <div class="container-fluid px-4 py-5">
        ...
        <div class="relative overflow-hidden rounded-xl bg-primary p-6 shadow-lg mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 relative z-10">
                <div>
                    <h2 class="text-3xl font-bold text-white">Danh Sách Bài Viết</h2>
                    <p class="mt-1 text-white/90">Quản lý tất cả các bài viết trong chương "{{ $chapter->name }}"</p>
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <a href="{{ route('admin.bookChapters.createPost', $chapter->id) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white py-2.5 px-4 text-center font-medium text-primary hover:bg-gray-100 transition-all duration-200 shadow-sm">
                        <span class="iconify" data-icon="mdi-plus"></span>
                        Tạo Bài Viết Mới
                    </a>
                    <a href="{{ route('admin.bookChapters.edit', $chapter->id) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white/20 py-2.5 px-4 text-center font-medium text-white hover:bg-white/30 transition-all duration-200">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
            ...
            <div class="flex items-center">
            <span class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 mr-2">
                <span class="iconify" data-icon="mdi-bookmark-outline"></span>
            </span>
                <span class="font-medium text-gray-700">{{ $chapter->name }}</span>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('posts.show', $post->slug) }}" class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none transition-colors" target="_blank">
                                            <span class="iconify mr-1" data-icon="mdi-eye"></span>
                                            Xem
                                        </a>
                                        <a href="{{ route('admin.posts.edit', $post->id) }}" class="inline-flex items-center px-2.5 py-1.5 border border-transparent shadow-sm text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none transition-colors">
                        ...
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <span class="iconify text-4xl text-gray-400 mb-3" data-icon="mdi-file-document-outline"></span>
                                    <h4 class="text-lg font-medium text-gray-600 mb-1">Chưa Có Bài Viết Nào</h4>
                                    <p class="text-gray-500 mb-4">Hãy tạo bài viết đầu tiên cho chương này</p>
                                    <a href="{{ route('admin.bookChapters.createPost', $chapter->id) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-primary py-2 px-4 text-center font-medium text-white hover:bg-primary/90 transition-all duration-200 shadow-sm">
                                        <span class="iconify" data-icon="mdi-plus"></span>
                                        Tạo Bài Viết Mới
                                    </a>
                                </div>
    ...
    <div class="fixed bottom-6 right-6 z-50">
        <a href="{{ route('admin.bookChapters.createPost', $chapter->id) }}" class="flex items-center justify-center w-14 h-14 rounded-full bg-primary text-white shadow-lg hover:bg-primary/90 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2">
