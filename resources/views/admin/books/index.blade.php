...
@section('content')
    <div class="container-fluid px-4 py-5">
        ...
        <div class="relative overflow-hidden rounded-xl bg-primary p-6 shadow-lg mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 relative z-10">
                <div>
                    <h2 class="text-3xl font-bold text-white">Quản Lý Sách</h2>
                    <p class="mt-1 text-white/90">Tổng hợp và quản lý tất cả sách trên website</p>
                </div>
                <div class="flex flex-wrap items-center gap-3">
                    <a href="{{ route('admin.books.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white py-2.5 px-4 text-center font-medium text-primary hover:bg-gray-100 transition-all duration-200 shadow-sm">
        ...
        <div class="rounded-xl border border-stroke bg-white p-4 shadow-md mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h3 class="text-lg font-semibold text-black flex items-center">
                    <span class="iconify mr-2 text-primary" data-icon="mdi-book-search-outline"></span>
                    Tìm Kiếm Sách
                </h3>
                                ...
                                <tr class="bg-gray-50">
                                    <td colspan="4" class="px-6 py-3">
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <span class="text-xs font-medium text-primary">{{ $book->group->category->name }}</span>
                                                <h4 class="text-sm font-semibold text-gray-800">{{ $currentGroup->name }}</h4>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <div class="text-xs text-gray-500">{{ $book->slug }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('admin.books.chapters', $book->id) }}" class="inline-flex items-center px-2.5 py-1.5 border border-transparent shadow-sm text-xs font-medium rounded text-white bg-blue-600 hover:bg-blue-700 focus:outline-none transition-colors">
                                            <span class="iconify mr-1" data-icon="mdi-format-list-bulleted"></span>
                                            Chương
                                        </a>
                                        <a href="{{ route('books.show', $book->slug) }}" class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none transition-colors" target="_blank">
                                            <span class="iconify mr-1" data-icon="mdi-eye"></span>
                                            Xem
                                        </a>
                                        <a href="{{ route('admin.books.edit', $book->id) }}" class="inline-flex items-center px-2.5 py-1.5 border border-transparent shadow-sm text-xs font-medium rounded text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none transition-colors">
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <span class="iconify text-4xl text-gray-400 mb-3" data-icon="mdi-book-open-variant"></span>
                                    <h4 class="text-lg font-medium text-gray-600 mb-1">Chưa Có Sách Nào</h4>
                                    <p class="text-gray-500 mb-4">Hãy thêm sách đầu tiên của bạn</p>
                                    <a href="{{ route('admin.books.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-primary py-2 px-4 text-center font-medium text-white hover:bg-primary/90 transition-all duration-200 shadow-sm">
                                        <span class="iconify" data-icon="mdi-plus"></span>
                                        Thêm Sách Mới
                                    </a>
                                </div>
    ...
    <div class="fixed bottom-6 right-6 z-50">
        <a href="{{ route('admin.books.create') }}" class="flex items-center justify-center w-14 h-14 rounded-full bg-primary text-white shadow-lg hover:bg-primary/90 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2">
