...
@section('content')
    ...
    <div class="container mx-auto p-4">
        <div class="bg-white dark:bg-boxdark rounded-lg shadow-md overflow-hidden">
            <!-- Header -->
            <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between">
                <div class="flex items-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7" />
                    </svg>
                    <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Footer Management</h1>
                </div>
            ...
            <!-- Success Message -->
            @if(session('success'))
                <div class="mx-6 mt-4 p-4 bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 dark:border-green-400 rounded-md">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <svg class="h-5 w-5 text-green-500 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="p-6">
                ...
                <div id="previewSection" class="mb-8">
                    <div class="flex items-center mb-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Footer Preview</h2>
                        <div class="ml-2 bg-gray-100 dark:bg-gray-700 text-xs px-2 py-1 rounded-full text-gray-500 dark:text-gray-400">
                        ...
                        <div class="grid {{ $gridClass }} gap-8">
                            @foreach($activeColumns as $column)
                                <div class="flex flex-col">
                                    <h4 class="text-purple-600 dark:text-purple-400 font-semibold mb-4 text-lg">{{ $column->title }}</h4>
                                    <ul class="space-y-3">
                                        @foreach($column->links->where('is_active', true) as $link)
                                            <li>
                                                <a href="{{ $link->url }}" class="text-gray-600 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 transition-colors duration-200">
                                                    {{ $link->title }}
                                                </a>
                                            </li>
                                        @endforeach
                ...
                <div id="manageSection" class="hidden">
                    <div class="flex items-center mb-4">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Manage Footer Columns</h2>
                        <div class="ml-2 bg-blue-100 dark:bg-blue-900/30 text-xs px-2 py-1 rounded-full text-blue-600 dark:text-blue-400">
                    ...
                    <div id="footer-columns" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        @foreach($columns as $column)
                            <div class="footer-column-item bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200" data-column-id="{{ $column->id }}">
                                <!-- Column Header -->
                                <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center cursor-move bg-gray-50 dark:bg-gray-700/50 rounded-t-lg">
                                    <div class="flex items-center">
                                    ...
                                    <div class="flex space-x-1">
                                        <button type="button" onclick="editColumn({{ $column->id }}, '{{ $column->title }}', {{ $column->is_active ? 'true' : 'false' }})"
                                                class="p-2 text-gray-500 hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors">
                                        ...
                                        <form action="{{ route('admin.footer.columns.destroy', $column) }}" method="POST" class="inline-block">
                                            ...
                                            <button type="button" onclick="confirmDelete(this.parentNode)"
                                                    class="p-2 text-gray-500 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
