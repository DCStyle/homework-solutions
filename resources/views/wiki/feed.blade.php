...
@php
    ...
    $title = 'Wiki Feed - Câu hỏi mới nhất' . (isset($category) ? ' về ' . $category->name : '') . (isset($bookGroup) ? ' trong ' . $bookGroup->name : '');
    $description = 'Khám phá những câu hỏi mới nhất từ cộng đồng. | ' . setting('site_description');
@endphp
        ...
        <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-10">
                <h1 class="text-3xl font-bold text-gray-900 mb-3">
                    @if(isset($category))
                        Câu hỏi <span class="text-primary">{{ $category->name }}</span> mới nhất
                    @elseif(isset($bookGroup))
                        Câu hỏi <span class="text-primary">{{ $bookGroup->name }}</span> mới nhất
                    @else
                        Câu hỏi mới nhất
                    @endif
                </h1>
                    ...
                    <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 sticky top-8">
                        <div class="p-5 border-b border-gray-100">
                            <h2 class="text-lg font-semibold text-gray-800 flex items-center">
                                <span class="iconify mr-2" data-icon="mdi-folder-multiple-outline"
                                      data-width="22"></span>
                                Danh Mục
                            </h2>
                        </div>
                            ...
                            <div>
                                <ul class="divide-y divide-gray-100">
                                    @forelse($latestQuestions as $question)
                                        <li>
                                            <a href="{{ route('wiki.show', [$question->category->slug ?? 'uncategorized', $question->slug]) }}"
                                               class="block p-4 hover:bg-gray-50 transition-colors">
                                                <p class="font-medium text-indigo-600 text-sm line-clamp-1">{{ $question->title }}</p>
                                                <div class="flex items-center mt-2 text-xs text-gray-500">
                                                <span class="inline-flex items-center">
                                                    <span class="iconify mr-1" data-icon="mdi-account"
                                                          data-width="14"></span>
                                                    {{ $question->user->name ?? 'Anonymous' }}
                                                </span>
                                                    <span class="mx-2">•</span>
                                                    <span class="inline-flex items-center">
                                                    <span class="iconify mr-1" data-icon="mdi-calendar"
                                                          data-width="14"></span>
                                    ...
                                    @empty
                                        <li class="p-4 text-center text-gray-500">
                                            <span class="iconify block mx-auto mb-2" data-icon="mdi-help-circle-outline"
                                                  data-width="24"></span>
                                            Chưa có câu hỏi nào.
                                        </li>
                                    @endforelse
                        ...
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                            ...
                            <div>
                                <ul class="divide-y divide-gray-100">
                                    ...
                                    @empty
                                        <li class="p-4 text-center text-gray-500">
                                            <span class="iconify block mx-auto mb-2" data-icon="mdi-chart-line"
                                                  data-width="24"></span>
                            ...
                            <div
                                class="p-4 bg-gradient-to-r from-indigo-50 to-purple-50 border-t border-gray-100 text-center">
                                <a href="{{ route('wiki.index') }}"
                                   class="inline-flex items-center text-sm font-medium text-indigo-600 hover:text-indigo-700">
                                    Đặt câu hỏi mới
                                    <span class="iconify ml-1" data-icon="mdi-arrow-right" data-width="16"></span>
